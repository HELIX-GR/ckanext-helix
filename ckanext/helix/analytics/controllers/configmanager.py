...
def setup(config):
    ...
    database_url = config.get(
        'ckanext.helix.analytics.database_url',
    ...
    logfile_pattern = config.get(
        'ckanext.helix.analytics.logfile_pattern',
        '/mnt/hgfs/logs/*.log')

    export_date_format = config.get(
        'ckanext.helix.analytics.export_date_format',
        '%Y-%m-%d')

    ha_proxy_datetime_format = config.get(
        'ckanext.helix.analytics.ha_proxy_datetime_format',
        '%d/%b/%Y:%H:%M:%S')

        ...
        return {
            ...
            initialize: function () 
            {
                ...
                var $list = $el.children('ul')
                    ...
                    .on('helix-item_editor:remove', ignore_event)
                    .on('helix-item_editor:create', ignore_event)
                    .on('helix-item_editor:enable', ignore_event)
                    .on('helix-item_editor:disable', ignore_event)
                ...
                for (var key in opts.terms) {
                    ...
                    $y.itemEditor(yopts)
                    $y.on('helix-item_editor:destroy', remove_self)
                }
            ...
            initialize: function () 
            {
                ...
                for (var key in opts.terms) {
                    ...
                    if ($item.length) {
                        ...
                        $item.on('helix-item_editor:destroy', handle_destroy)
                    } else {
                        $('<option/>', { value: key })
                            .text(title)
                            .appendTo($select)
        ...
        return {
            ...
            initialize: function () 
            {
                ...
                $list.addClass('edit-list-items')
                    ...
                    .on('helix-item_editor:enable', ignore_event)
                    .on('helix-item_editor:disable', ignore_event)
                    .on('helix-item_editor:remove', warn_unexpected_event) 
                    .on('helix-item_editor:destroy', warn_unexpected_event)
                ...
                var handle_remove = function (ev) {
                    ...
                    $items.eq(num_items - 1)
                        .one('helix-item_editor:destroy', function (ev1) {
                            $(this).remove()
                            num_items -= 1
                            return false
                        })
                ...
                $list.children('li').each(function (i, li) {
                    ...
                    $item.on('helix-item_editor:remove', handle_remove)
                    num_items += 1
                })
                ...
                $add_btn.on('click', function () {
                    ...
                    if (i == opts.maxlength) {
                        ...
                        var $item = $('<li/>')
                            ...
                            .on('helix-item_editor:remove', handle_remove)
                            .appendTo($list)
                        num_items += 1
                        setTimeout(function () {
                            $item.find(':input')
                ...
                $clear_btn.on('click', function () {
                    ...
                    module.el.find('.btn.go-clear-items').one('click', function () {
                        module.el.children('ol').children('li').each(function (i, item) {
                            $(item)
                                .one('helix-item_editor:destroy', function () {
                                    $(this).remove()
                                    num_items -= 1
                                    return false    
                                })
                ...
                $list.on('helix-item_editor:move-up', function (ev) {
                    var $item = $(ev.target),
                        i = $item.data('index')
                    assert(i > 0)

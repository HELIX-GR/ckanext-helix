...
class RasterStorer(plugins.SingletonPlugin):
    ...
    def before_map(self, map):
        ...
        map.connect("raster-publish", "/api/raster/publish/{resource_id}",
                    controller='ckanext.helix.storers.raster.controllers.import:RasterImportController', action='publish',
                    resource_id='{resource_id}')
        map.connect("raster-delete", "/api/raster/delete/{resource_id}",
                    controller='ckanext.helix.storers.raster.controllers.import:RasterImportController', action='delete',
                    resource_id='{resource_id}')
        map.connect("raster-finalize", "/api/raster/finalize/{resource_id}",
                    controller='ckanext.helix.storers.raster.controllers.import:RasterImportController', action='finalize',
                    resource_id='{resource_id}')
        return map
    ...
    def update_config(self, config):
        ...
        """
        plugins.toolkit.add_public_directory(config, global_config.get("ckanext.helix.rasterstorer.temp_dir", ""))
        plugins.toolkit.add_public_directory(config, 'public')
        plugins.toolkit.add_template_directory(config, 'templates')
        plugins.toolkit.add_resource('public', 'ckanext-helix-raster')

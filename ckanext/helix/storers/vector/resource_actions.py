...
from ckanext.helix.model.resource_ingest import (
    ResourceIngest, ResourceStorerType, IngestStatus)
from ckanext.helix.storers.vector.resources import (
...
def _make_default_context():
    ...
    return {
        ...
        'gdal_folder': config.get(
            'ckanext.helix.vectorstorer.gdal_folder'),
        'temp_folder': config.get(
            'ckanext.helix.vectorstorer.temp_dir'),
    }
...
def _make_geoserver_context():
    ...
    return {
        'url':
            config['ckanext.helix.vectorstorer.geoserver.url'].rstrip('/'),
        'api_url':
            config['ckanext.helix.vectorstorer.geoserver.api_url'].rstrip('/'),
        'reload_url':
            config.get('ckanext.helix.vectorstorer.geoserver.reload_url'),
        'workspace':
            config['ckanext.helix.vectorstorer.geoserver.workspace'],
        'username':
            config['ckanext.helix.vectorstorer.geoserver.username'],
        'password':
            config['ckanext.helix.vectorstorer.geoserver.password'],
        'datastore':
            config['ckanext.helix.vectorstorer.geoserver.datastore'],
    }

def _make_mapserver_context():
    return {
        'url':
            config['ckanext.helix.vectorstorer.mapserver.url'].rstrip('/'),
        'mapfile_folder':
            config['ckanext.helix.vectorstorer.mapserver.mapfile_folder'],
        'templates_folder':
            config['ckanext.helix.vectorstorer.mapserver.templates_folder'],
    }
...
def _make_backend_context():
    ...
    if _geoserver_context is not None and _mapserver_context is not None:
        try:
            _default_publishing_server = config['ckanext.helix.vectorstorer.default_publishing_server']
        except KeyError:

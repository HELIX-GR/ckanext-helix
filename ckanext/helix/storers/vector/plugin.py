...
import ckanext.helix.storers.vector as vectorstorer
from ckanext.helix.storers.vector import resource_actions
from ckanext.helix.storers.vector.resources import (
    DBTableResource, WMSResource, WFSResource) 
from ckanext.helix.storers.vector.lib.template_helpers import (
...
class VectorStorer(p.SingletonPlugin):
    ...
    def before_map(self, map):
        controllers_base = "ckanext.helix.storers.vector.controllers."
        ...
        map.connect(
            'search_epsg',
            '/api/helix/search_epsg',
            controller=vectorController,
            action='search_epsg')
        
        map.connect(
            'search_encoding',
            '/api/helix/search_encoding',
            controller=vectorController,
            action='search_encoding')
        
        map.connect(
            'validation_check',
            '/api/helix/vector/validation_check',
            controller=vectorController,
            action='validation_check')

        map.connect(
            'vector_ingest',
            '/api/helix/vector/ingest/{resource_id}',
            controller=vectorController,
            action='ingest',
            resource_id='{resource_id}')
    ...
    def update_config(self, config):
        ...
        p.toolkit.add_resource('public', 'ckanext-helix-vector')

...
from ckanext.helix.lib.metadata import adapter_registry 
from ckanext.helix.lib.metadata import schemata 
from ckanext.helix.lib.metadata import types
from ckanext.helix.lib.cli import CommandDispatcher

subcommand = CommandDispatcher.subcommand

class Command(CommandDispatcher):
    '''This is a Paster command for several helix-related subcommands
    
    >>> paster [PASTER-OPTS] helix --config FILE [--setup-app] [COMMAND] [COMMAND-OPTS] [COMMAND-ARGS]

    Invoke with '?' as a COMMAND in order to get a list of available commands.
    '''

    summary = '''This is a Paster command for several helix-related subcommands'''
    usage = __doc__
    group_name = 'ckanext-helix'
    max_args = 15
    min_args = 0
    ...
    def setup_extension(self, opts, *args):    
        '''Setup helix extension (create tables, populate with initial data).
        '''
        
        import ckan.model.meta as meta
        import ckanext.helix.model as helix_model
        ...
        if opts.dry_run:
            ...
            self.logger.info('Creating tables at %s@%s: \n%s' % (
                ...
                ', '.join(helix_model.Base.metadata.tables.keys())))
        else:
            helix_model.Base.metadata.create_all(bind=meta.engine)
            helix_model.post_setup(engine=meta.engine)

        self.logger.info('Setup complete')
    
    @subcommand('cleanup', options=options_config['cleanup'])
    def cleanup_extension(self, opts, *args):    
        '''Cleanup helix extension.
        '''
        ...
        if opts.dry_run:
            ...
            helix_model.pre_cleanup(engine=meta.engine)
            helix_model.Base.metadata.drop_all(bind=meta.engine)
    ...
    def print_formatter_info(self, opts, *args):
        ...
        from ckanext.helix.lib.metadata.util import (
            iter_object_schemata, iter_field_adaptee_vectors)
        from ckanext.helix.lib.metadata import formatters
    ...
    def export_package_translation(self, opts, *args):
        ...
        '''
        from ckanext.helix.model import PackageTranslation       
    ...
    def import_package_translation(self, opts, *args):
        ...
        from ckanext.helix.lib.metadata.i18n import package_translation
    ...
    def migrate_db_to_unicode(self, opts, *args):
        ...
        from ckanext.helix.lib.metadata import dataset_types
    ...
    def analyze_logs(self, opts, *args):
        ...
        from ckanext.helix.analytics.controllers import configmanager
        from ckanext.helix.analytics.controllers.dbservice import (DbReader, DbManager)
        from ckanext.helix.analytics.controllers.util.system import SystemInfo
        from ckanext.helix.analytics.controllers.log_trimmer import LogTrimmer
        from ckanext.helix.analytics.controllers.parsers.habboxaccessparser import HABboxAccessParser
        from ckanext.helix.analytics.controllers.parsers.hacoveragebandparser import HACoverageBandParser
        from ckanext.helix.analytics.controllers.parsers.hausedcoveragesparser import HAUsedCoveragesParser
        from ckanext.helix.analytics.controllers.parsers.haparser import HAParser
        from ckanext.helix.analytics.controllers.parsers.haservicesaccessparser import HAServicesAccessParser
